...
AmclNode::createLaserObject()
{
  ...
  RCLCPP_INFO(get_logger(), "it is IAR_AMCL Beam Model");

  return new iar_amcl::BeamModel(
    z_hit_, z_short_, z_max_, z_rand_, sigma_hit_, lambda_short_, max_beams_, map_);
...
AmclNode::initOdometry()
{
  ...
  motion_model_ = std::unique_ptr<iar_amcl::DifferentialMotionModel>(
    new iar_amcl::DifferentialMotionModel(alpha1_, alpha2_, alpha3_, alpha4_)
  );
